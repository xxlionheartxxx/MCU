...
BOOL Conference::AddMember(ConferenceMember * memberToAdd, BOOL addToList)
{
  ...
  if(UseSameVideoForAllMembers())
  {
      ...
      )
    {
      ...
      //CLogger::getLogger()->Log("hello a3aaaa 2 %d",memberToAdd->get_Qos());
  ...
  //CLogger::getLogger()->Log("hello a3a");
...
void Conference::ReadMemberAudio(ConferenceMember * member, const uint64_t & timestamp, void * buffer, int amount, int sampleRate, int channels)
{
  ...
  {
    ConferenceAudioConnection * conn = it.GetObject();
    if(conn->GetID() == member->getVideoType()){
      AnyBodyHere = 1;
    }
  }

  // truyen audio tuong ung
  if(member->getVideoType() == 500 || AnyBodyHere ==0){
    for(MCUAudioConnectionList::shared_iterator it = audioConnectionList.begin(); it != audioConnectionList.end(); ++it)
    {
      ConferenceAudioConnection * conn = it.GetObject();
    ...
    //CLogger::getLogger()->Log("id %d muon xem video o %d",member->GetID(), member_relay->getVideoType() );
    for(MCUAudioConnectionList::shared_iterator it = audioConnectionList.begin(); it != audioConnectionList.end(); ++it)
    {
      ConferenceAudioConnection * conn = it.GetObject();
      if(conn->GetID() != member->getVideoType()){
      ...
      //CLogger::getLogger()->Log("conn %d \t member %d, count %d",conn->GetID(), member->GetID(),count = 0);
      BOOL skip = moderated&&muteUnvisible;
      if(skip)
      {
        for(MCUVideoMixerList::shared_iterator it = videoMixerList.begin(); it != videoMixerList.end(); ++it)
...
//////////////////////////////////////LDLac//////////////////////////////
//--------LDLac code
MCUSimpleVideoMixer * Conference::find_VideoMix (long mixerNumber_test) {
  MCUSimpleVideoMixer * mixer = manager.FindVideoMixerWithLock(this, mixerNumber_test);
  return mixer;
}
//--------LDLac code
/////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////////////////////
void Conference::ReadMemberVideo(ConferenceMember * member, void * buffer, int width, int height, PINDEX & amount, unsigned type)//LDLac code
{
  if(videoMixerList.GetSize() == 0)
    return;
  ...
  mixer->ReadFrame(*member, buffer, width, height, amount, type);// LDLac code
  mixer->Unlock();
}
...
{
  member_info.videoType = 500; //LDLac code
  member_info.code_Qos = -10;
  member_info.change = 0;
...
void ConferenceMember::ReadVideo(void * buffer, int width, int height, PINDEX & amount)
{
  ...
  if(conference != NULL)
  {
    //CLogger::getLogger()->Log("ID " + GetName().operator std::string());
    //CLogger::getLogger()->Log("Id %d", videoType);
    if(conference->UseSameVideoForAllMembers())
      conference->ReadMemberVideo(this, buffer, width, height, amount,member_info.videoType);///LDLac code
    else if(videoMixer != NULL)
      videoMixer->ReadFrame(*this, buffer, width, height, amount,member_info.videoType);  ///LDLac code
  }
}
