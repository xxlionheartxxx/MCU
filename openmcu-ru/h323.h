...
class MCUH323Connection : public H323Connection
{
  ...
  public:
    ...
    int rtpInputBytes;
    ///-------------LDLac code 1/4
    //void setCFMBVideoType(unsigned type) {conferenceMember->setVideoType(type);}
    //unsigned getCFMBVideoType() {return conferenceMember->getVideoType();}

    float mcu_connection_get_current_quality(){
      float audio_rating=-1.0;
      float video_rating=-1.0;
      float result =10.0 ;
      if (audioTransmitChannel){
        audio_rating=audioTransmitChannel->mcu_channel_get_quality()/5.0;
      }
      if (videoTransmitChannel){
        video_rating=videoTransmitChannel->mcu_channel_get_quality()/5.0;
      }
      if (audio_rating<0 && video_rating<0){
        result=-1;
      }
      ...
      //CLogger::getLogger()->Log("audio_rating %f video_rating %f",audio_rating,video_rating);
      return result;
    }
    ///-------------LDLac code 1/4
...
struct Count_R_Info {
  ...
  float tb_change_R = 0;
  time_t sum_quality_timer = 0;// check current
  time_t last_decrease = 0;
};
...
class MCUPVideoOutputDevice : public PVideoOutputDevice
{
  ...
  protected:
    Count_R_Info info_count;//LDLac code
    MCUH323Connection & mcuConnection;
};
