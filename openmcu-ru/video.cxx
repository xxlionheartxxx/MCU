...
{
  if(WidthMax < width && WidthMax != 0) { width = WidthMax;}
  if(HeightMax < height && HeightMax != 0) {height = HeightMax;}
...
{
  //-------LDLac code
  ConferenceMember * member_relay=NULL;
  ConferenceMember * member_now = mcuConnection.GetConferenceMember();
  int d = member_now->GetID();
  // lay conference ra
  Conference * cf = mcuConnection.GetConference();
  /*//----------- lay memberlist cua conference
  MCUMemberList & memberList = cf->GetMemberList();
    for(MCUMemberList::shared_iterator it2 = memberList.begin(); it2 != memberList.end(); ++it2)
    {
      ConferenceMember * member = *it2;
      int c = member->GetID();  // lay id cua truyen stream
      CLogger::getLogger()->Log("ID %d co QOS la %d",member->GetID(), member_now->get_Qos());
    }
  *///
  if(HeightMax == 0 && WidthMax == 0) {GetFrameSize(WidthMax, HeightMax);}
  int code_relay = member_now->get_Qos();
  ...
  if(code_relay != -1){
    ...
    ///
    //CLogger::getLogger()->Log("ID %d co QOS la %d, fps la %d", member_now->GetID(), code_relay, GetFrameRate());
  }
  //----------- lay memberlist cua conference end
  //-------end LDLac code

  //CLogger::getLogger()->Log("ID %d co videotype %d", member_now->GetID(), member_now->getVideoType());
...
MCUPVideoOutputDevice::~MCUPVideoOutputDevice()
{}
...
// ----------LDLac code
int Get_code(float R_Qos){
	if(R_Qos > 4.0) return 4;
	if(R_Qos > 3.0) return 3;
	if(R_Qos > 2.0) return 2;
	if(R_Qos > 1.0) return 1;
  if(R_Qos > 0.0) return 0;
	return -1;
}
//----------end LDLac code
                                              ...
                                              BOOL /*endFrame*/)
{
  ...
  static float tb_change_R = -1.0;
  static int count_increase = 0;
  static time_t sum_quality_timer = time(NULL);// check current
  static time_t last_decrease = 0;*/
  time_t NOW;
  NOW = time(NULL);
  float tb_current_R = -1.0;
  ...
  if (difftime(NOW,info_count.sum_quality_timer) >= _CHECK_INTERVAL_ ){
    // calculate average
    tb_current_R = info_count.sum_R/info_count.count_sum;
    // first time
    if (info_count.tb_change_R == 0){
       info_count.tb_change_R = tb_current_R;
       info_count.sum_quality_timer = time(NULL);// check current
       info_count.last_decrease = time(NULL);
    } else {
      int code_of_R =  Get_code(tb_current_R);
      int pre_code_of_R = Get_code(info_count.tb_change_R);
      //CLogger::getLogger()->Log("member id %d co QOS %f, code %d",member->GetID(), tb_current_R);
      //member->set_and_get_Qos(code_of_R, 1);
      if (code_of_R != pre_code_of_R) {
        //CLogger::getLogger()->Log("member id %d co QOS %d",member->GetID(), code_of_R);
        //------Neu network up
        if(code_of_R > pre_code_of_R && info_count.count_increase == 3 ){
          //Tang fps vs res
          //CLogger::getLogger()->Log("TANG fps and res vi code %d va precode %d", code_of_R, pre_code_of_R );
          if(pre_code_of_R != -1){
            member->set_Qos(pre_code_of_R + 1);
            info_count.tb_change_R++;
          } else {
            member->set_Qos(code_of_R);
            info_count.tb_change_R= tb_current_R;
          }
          // reset parameters
          info_count.count_increase = 0;
        } else if(code_of_R > pre_code_of_R){
           info_count.count_increase++;
        } else  info_count.count_increase = 0;// tang bien dem network up

        //-----Neu network down
        if(code_of_R < pre_code_of_R && (NOW - info_count.last_decrease) >_CHECK_INTERVAL_LAST_CHANGE_DECREASE){
          // giam fps vs res
          //CLogger::getLogger()->Log("giam fps and res vi code %d va precode %d", code_of_R, pre_code_of_R );
          member->set_Qos(code_of_R);
          // reset parameters
          info_count.last_decrease = NOW;
          info_count.count_increase = 0;
          info_count.tb_change_R = tb_current_R;
        } else if(code_of_R < pre_code_of_R) info_count.count_increase = 0;
      } else {
        info_count.count_increase = 0;
      }
    }
    info_count.sum_quality_timer = NOW;
    info_count.sum_R = 0.0f;
    info_count.count_sum = 0;
  }
  //----------- LDLac cdoe
...
BOOL MCUSimpleVideoMixer::ReadFrame(ConferenceMember & member, void * buffer, int width, int height, PINDEX & amount, unsigned type) //LDLac code
{
