...
void MCU_RTPChannel::Transmit()
{
  ...
  while(1)
  {
    BOOL retval = FALSE;
    //LDLac pha
    //CLogger::getLogger()->Log("id dang truyen la %d", get_id_from_channel());
    // setup cache
                         ...
                         )
{
  ...
  srtp_secured = FALSE;
  ///////----LDLac code
  packet_info.last_ext_seq_num =-1;
  packet_info.last_packet_sent_count_num = -1;
  packet_info.last_cum_loss_num =-1;
  /*last_ext_seq_num =-1;
  last_packet_sent_count_num =-1;
  last_cum_loss_num=-1;*/
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
///----------LDLac code 1/4
float inter_jitter_rating(float inter_jitter){
	float tmp=inter_jitter/0.2f;//WORSE_JITTER;
	if (tmp>1) tmp=1;
	return 1.0f-(0.3f*tmp);
}

float rt_prop_rating(float rt_prop){
	float tmp=rt_prop/5.0f;//WORSE_RT_PROP;
	if (tmp>1) tmp=1;
	return 1.0f-(0.7f*tmp);
}

float loss_rating(float loss){
...
float compute_rating(float loss_rate, float inter_jitter, float late_rate, float rt_prop){
  //CLogger::getLogger()->Log("loss_rating %f \t inter_jitter_rating %f \t loss_rating %f\t, rt_prop_rating %f",loss_rating(loss_rate),inter_jitter_rating(inter_jitter),loss_rating(late_rate),rt_prop_rating(rt_prop));
	return loss_rating(loss_rate)*inter_jitter_rating(inter_jitter)*loss_rating(late_rate)*rt_prop_rating(rt_prop);
}

float compute_rtt(PTimeInterval current, PTimeInterval lrr, PTimeInterval dlrr){
  float rtt = 0;
  PTime now;
  int now_time = ((now.GetTimeInSeconds()+SecondsFrom1900to1970) <<16) | ((now.GetMicrosecond()*4294)>>16);
  if (lrr.GetInterval()!=0 && dlrr.GetInterval()!=0){
		/*we cast to int32_t to check for crazy RTT time (negative)*/
		int rtt_frac=(now_time-lrr.GetInterval()-dlrr.GetInterval());
    //CLogger::getLogger()->Log("rtt %d \t now_time %d  \t lrr %d\t dlrr %d\n", rtt_frac, now_time,lrr.GetInterval(),dlrr.GetInterval());
		if (rtt_frac>=0){
			rtt_frac/=65536.0;
			rtt=(float)rtt_frac;
      //CLogger::getLogger()->Log("now_time %d  \t lrr %d\t dlrr %d\n", now_time,lrr.GetInterval(),dlrr.GetInterval());
...
{
  ////////////LDLac code
  static float RATING_SCALE = 5.0;
  static int min_packet_count_interval =5;
  static int min_time_ms_interval =30;
  //static PTimeInterval last_estimate_time_ms;
  unsigned size = frame.GetPayloadSize();
  int clock_rate = mcu_session_get_clock_rate();
  //CLogger::getLogger()->Log("hello id %d",mcu_session_get_clock_rate());
  //CLogger::getLogger()->Log("hello %d \t %d \t %d",packet_info.last_ext_seq_num,packet_info.last_packet_sent_count_num,packet_info.last_cum_loss_num);
  if(frame.GetPayloadType() == RTP_ControlFrame::e_SenderReport && size >= sizeof(RTP_ControlFrame::SenderReport))
  {
    ...
    if(ra.GetSize() > 0){
      ...
      ///--------------LDLac code tinh R cho luong server----------->client
      int cum_loss = ra[ra.GetSize()-1].totalLost;
      int extseq = ra[ra.GetSize() - 1].lastSequenceNumber;
      float inter_jitter = ra[ra.GetSize()-1].jitter/clock_rate;
      int diff_total_outgoing = packetsSent- packet_info.last_packet_sent_count_num;
      int diff;
      float loss_rate;
      PTimeInterval current = PTimer::Tick();
      if(packet_info.last_ext_seq_num == -1 ){
        packet_info.last_ext_seq_num = extseq;
        packet_info.last_cum_loss_num = cum_loss;
        packet_info.last_estimate_time_ms = PTimer::Tick();
      }
      diff=extseq-packet_info.last_ext_seq_num;
      if(diff < 0 || diff > min_packet_count_interval *100){
        packet_info.last_ext_seq_num=extseq;
        packet_info.last_cum_loss_num=cum_loss;
        packet_info.last_packet_sent_count_num=packetsSent;
      }else if (diff > min_packet_count_interval && (current - packet_info.last_estimate_time_ms).GetInterval() >=min_time_ms_interval){
        float R =0.0;
        int new_losses = cum_loss - packet_info.last_cum_loss_num;
        float tmp = (diff_total_outgoing - new_losses)*1.f/ diff_total_outgoing;
        loss_rate = 100.f * (1.f - (tmp>0 ? tmp: 0));
        //rt_prop = T3 - LSR - DLSR;
        ...
        packet_info.last_estimate_time_ms = current;
        packet_info.last_ext_seq_num=extseq;
        packet_info.last_cum_loss_num=cum_loss;
        packet_info.last_packet_sent_count_num=packetsSent;
      }
  ...
  else
  {
    ...
    if(sequenceNumber == expectedSequenceNumber)
    {
      ...
      if(!frame.GetMarker())
      {
        ...
        ///---------------LDLac code
        //diff *= 8;
        int variance = (diff - lastTransitTime);
        lastTransitTime = diff;
        if (variance < 0)
        {
          variance *= (-1);
          //CLogger::getLogger()->Log("id dang truyen la %d", (int)jitterLevel);
        }
        //jitterLevel += variance - ((jitterLevel+8) >> 4);
        jitterLevel += (variance - (int)jitterLevel) /16.0;
        ///---------------end LDLac code
