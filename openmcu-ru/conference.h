...
class ConferenceMember : public PObject
{
...
#endif
  //------------------LDLac code--------------
  unsigned getVideoType() { return member_info.videoType;}
  void setVideoType(unsigned type) {
    member_info.videoType = type;
    //CLogger::getLogger()->Log("thay gia chua cha  %d va  videoType la %d", type, videoType);
  }
  // Set Qos vs Get Qos dua vao bool_tmp
  void set_Qos(int code){
    member_info.code_Qos = code;
    member_info.change = 1;
  }

  int get_Qos(){
    if(member_info.change){
      member_info.change =0;
      return member_info.code_Qos;
    }
    return -1;
  }
  //------------------end LDLac code--------------
  ...
  protected:
    //loai video truyen ve LDLac code
...
#if MCU_VIDEO
    virtual void ReadMemberVideo(ConferenceMember * member, void * buffer, int width, int height, PINDEX & amount, unsigned type); // LDLac code
