...
BOOL MCUH323Connection::OpenAudioChannel(BOOL isEncoding, unsigned /* bufferSize */, H323AudioCodec & codec)
{
  ...
  if(isEncoding)
  {
    ...
    //LDLac pha. set id cho channel
    if(audioTransmitChannel){
      audioTransmitChannel->set_id_for_channel(conferenceMember->GetID());
    }
    // setup cache
    if(enableCache)
    {
...
BOOL MCUH323Connection::OpenVideoChannel(BOOL isEncoding, H323VideoCodec & codec)
{
  ...
  if(isEncoding)
  {
    ...
    /*// LDLac pha vs set id cho channel
    if(videoTransmitChannel){
      videoTransmitChannel->set_id_for_channel(conferenceMember->GetID());
    }//*/
    // Режим с кэшированием
    if(cacheMode == 20)///LDLac pha
    {
      //CLogger::getLogger()->Log("hello %d", conferenceMember->GetID());
...
void MCUH323Connection::OnUserInputString(const PString & str)
{
  ...
  if(code != "" && MCUConfig("Control Codes").HasKey(code))
  {
    ...
    int command_length = a.length();
    // lay conference ra
    Conference * cf = GetConference();
    ...
    command.append(a,0,17);
    if(command == "LDLacVideoType = "){
      code_linphone.append(a,17,1);
      //frame mix
      if(code_linphone == "M"){
      ...
      }else if(code_linphone >= "0" && code_linphone <= "9"){ //frame rieng
        int videotype = atoi(&code_linphone[0]);
        MCUSimpleVideoMixer * mix_video = cf->find_VideoMix(conferenceMember->GetVideoMixerNumber());
        if(mix_video){
          MCUVMPList::shared_iterator it = mix_video->VMPFind(videotype);
          MCUVMPList::shared_iterator it2 = mix_video->VMPFind(videotype*51234 + 11234213);// thuc the ko co
          if(it!=it2){ // it==it2 la it ko co, neu it!=it2 thi la co
            int id_member = it->id;
            //CLogger::getLogger()->Log("hello");
            conferenceMember->setVideoType(id_member);
            //if(videoTransmitChannel)
            //  videoTransmitChannel->set_IDconferenceMember(id_member);
            CLogger::getLogger()->Log("hello %d muon xem %d",conferenceMember->GetID(), id_member);
          }
        }
      }
    } else conferenceMember->SendUserInputIndication(dtmfBuffer);
    //---------------------end LDLac code
  }
  dtmfBuffer = "";
}
